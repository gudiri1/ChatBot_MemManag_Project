...
ChatLogic::~ChatLogic()
{
    ...
    // TASK 3
    /* Delete takes place internally through unique pointers use
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        {
                            // get iterator on incoming and outgoing node via ID search    TASK 3
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                rootNode = (*it).get(); // assign current node to root     Task3   use .get() method to avoid creating another copy of unique pointer because as you know creating a copy of a unique pointer is not allowed.
            }
            else
            {
